...
public class TimeUtil {
    ...
    public static String getNomMesEspanhol(Date date) {
        return getNomMesEspanhol(new Integer(TimeUtil.getFecha2("MM", date)) - 1);
    }

    public static String getNomMesEspanhol(Integer numMes) {
        String m = "";
    ...
    public static String getTiempoDuracion(Date fechaIni, Date fechaFin) {
        ...
        if (fechaIni != null && fechaFin != null) {
            return new TimeUtil().restaFechaString(new TimeUtil().getFecha(formatFecha, fechaIni), new TimeUtil().getFecha(formatFecha, fechaFin));
        }
        return null;
    }
    ...
    public static String getTiempoDuracion(String fechaIni, String fechaFin) {
        if (fechaIni != null && fechaFin != null) {
            return new TimeUtil().restaFechaString(fechaIni, fechaFin);
        }
        return null;
    }
    ...
    public static int cantidadDiaEntreFechas(String dia, Date fechaIni, Date fechaFin) {
        ...
        while (!finCorte) {
            String diaEntra = new TimeUtil().getDiaEspanol(fechaAux);
            ...
            fechaAux.setTime(new TimeUtil().sumarDias(cIni.getTime(), cont).getTime());
    ...
    public static List<Date> listFechasEntreCortes(Date fechaIni, Date fechaFin) {
        ...
        try {
            while (!finCorte) {
                ...
                Date fechaIncrementa = new Date(new TimeUtil().sumarDias(cIni.getTime(), cont).getTime());
    ...
    public static List<Date> corteSemanal(String dia, Date fechaIni, Date fechaFin) {
        ...
        while (!finCorte) {
            ...
            fechaAux.setTime(new TimeUtil().sumarDias(fechaIni, cont).getTime());
            cont++;
        }

        for (int i = 0; i < lstF.size(); i++) {
            Date dFinal = new Date(new TimeUtil().finalDay(lstF.get(i)).getTime());
            Date nFecha = new Date(dFinal.getTime());
            lstF.set(i, nFecha);
        }

        fechaFinAux.setTime(new TimeUtil().finalDay(fechaFinAux).getTime());
    ...
    public static List<Date> corteQuincenal(Date fechaIni, Date fechaFin) {
        ...
        while (!finCorte) {
            ...
            int diaEntra = new TimeUtil().getNumDia(fechaAux);
            int numMes = new TimeUtil().getNumMes(fechaAux);
    ...
    public static List<Date> corteMensual(Date fechaIni, Date fechaFin) {
        ...
        int diaDesde = new TimeUtil().getNumDia(fechaIni);
        int diaUltimo = getUltimoDiaDelMes(fechaIni);

        int diasDif = diaUltimo - diaDesde;

        Date fechaCorte = new Date(new TimeUtil().sumarDias(fechaIni, diasDif).getTime());
        fechaCorte.setTime(new TimeUtil().finalDay(fechaCorte).getTime());
        ...
        while (new TimeUtil().getNumMes(fechaCorte) != new TimeUtil().getNumMes(fechaFin)) {

            diaDesde = new TimeUtil().getNumDia(fechaCorte);
            diaUltimo = getUltimoDiaDelMes(fechaCorte);

            diasDif = diaUltimo - diaDesde;

            Date fechaFinMes = new Date(new TimeUtil().sumarDias(fechaCorte, diasDif).getTime());
            fechaCorte.setTime(new Date(new TimeUtil().finalDay(fechaFinMes).getTime()).getTime());
    ...
    public static void main(String args[]) {
        String tim = new TimeUtil().getFechaHora();
        System.out.println(tim.replaceAll(" ", "_").replaceAll(":", "_").substring(0, tim.length() - 3));
        System.out.println(new TimeUtil().getHora());
        System.out.println(new TimeUtil().getTiempo("yyyy-MM-dd HH:mm:ss"));
        String[] res = new TimeUtil().restaFecha("2007-12-31 23:59:59", "2008-01-01 00:01:45");
        System.out.println(res[3] + " dias," + res[2] + " horas," + res[1] + " minutos," + res[0] + " segundos");
        System.out.println("SUMANDO MINUTOS: " + new TimeUtil().sumarMinutos("06:00:00", 62));
        //System.out.println("PERIODO ACTUAL: "+new Tiempo().getPrriodoActual());
        System.out.println("DIA DE LA SEMANA=" + new TimeUtil().getDiaEspanol(new Date()));

        String[] m = new TimeUtil().getNomMeses("2010-01-01", "2010-04-02");
